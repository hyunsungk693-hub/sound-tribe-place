...
const FirstRehearsal = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: job } = await db.from("posts").select("user_id, area, venue, title").eq("id", app.job_id).maybeSingle();
      ...
      // 양측 가능 시간 교집합
      const { data: profs } = await db.from("profiles").select("user_id, available_times").in("user_id", [applicantId, ownerId]);
      const times = (profs || []).map((p: any) => new Set<string>(p.available_times || []));
      if (times.length === 2) {
        setCommonTimes([...times[0]].filter((t) => times[1].has(t)));
      }
            ...
            <div className="glass-card p-4 mb-4">
              ...
              {commonTimes.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {commonTimes.map((t) => <span key={t} className="text-[11px] font-medium px-2.5 py-1 rounded-full bg-primary/10 text-primary">{t}</span>)}
                </div>
              ) : (
